...
<html lang="en">
  ...
  <body>
	...
	$val = new Valuation();
	$disp = new DisplayMongo();
		

	$urls = array();
	if($_SESSION['url'] !== null){array_push($urls, $_SESSION['url']);}
	if($_SESSION['url1'] !== null){array_push($urls, $_SESSION['url1']);}
	if($_SESSION['url2'] !== null){array_push($urls, $_SESSION['url2']);}
	if($_SESSION['url3'] !== null){array_push($urls, $_SESSION['url3']);}
	//$word = $obj->getWords($url);
	...
	foreach($urls as $url){
		...
		if($found == 1){
			...
			$query = array('market' => 'recession');
			$keys = $disp->displayKeys($query);
			$query1 = array('market' => 'test');
			$keys1 =$disp->displayKeys($query1);
			$seg1 = $val->coefficient($nouns, $keys);
			$seg2 =$val->coefficient($nouns, $keys1);
			...
			echo "</div><hr />
			<div class\"row\"><h2>Possible Markets</h2>
			<div class=\"col-lg-6\">
			<table class=\"table table-hover\">
			<tr><th>Market Segment</th><th>Relevance Score</th></tr>
			<tr><td>Recession Market</td><td>{$seg1}</td></tr>
			</table>
			</div>
			<div class=\"col-lg-6\">
			<table class=\"table table-hover\">
			<tr><th>Market Segment</th><th>Relevance Score</th></tr>
			<tr><td>Test Market</td><td>{$seg2}</td></tr>
			</table>
			</div>
			</div>
			...
			<div class=\"col-lg-2\">
			<a class=\"btn btn-lg btn-primary\" href=\"/improve.php\" role=\"button\">Improve</a></div>
			<div class=\"col-lg-5\"></div></div>";
			$storing = implode(",",$nouns);
			$entry = array("url"=>$url, "keywords"=>$storing, "nouns"=>$nouns, "type"=>"web");
			$api->addDoc($entry);
		}else{
